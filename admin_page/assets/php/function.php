<?php
	...
	function check($email, $psswd, $id){
		...
		if($userResult->num_rows > 0) return 1;
		else if($adminResult->num_rows > 0) return 2;
		else if($wrongpassResult->num_rows > 0) return 3;
		else if($wrongpassadResult->num_rows > 0) return 4;
		else return 0;
	}
	...
	function getinf($email){
		global $conn;
		$user = $conn->query("SELECT * FROM users WHERE email=?");
		$admin = $conn->query("SELECT * FROM admins WHERE email=?");
	...
	function getall(){
        ...
        ?>
            <tr>
                ...
                <?php if ($row['status'] == 0):?><td id="status1"><ion-icon name="radio-button-off-outline"></ion-icon><span>Processing</span></td>
                    <?php else:?><td id="status2"><ion-icon name="radio-button-on-outline"></ion-icon><span>Completely</span></td>
                <?php endif;?>
                <td>
                    <a href="index.php?id=<?php echo $row['movie_id']; ?>" 
						class="update"><i class="fa-solid fa-pen-to-square"></i>Update</a>|
                    <a href="result.php?act=delete&id=<?php echo $row['movie_id']; ?>"
                        class="remove" onclick="return confirm('Are you sure want to delete this movie?')"><i class="fa-solid fa-trash"></i>Remove</a>
                </td>
            </tr>
        <?php
        }
